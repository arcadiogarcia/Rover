...
namespace zRover.WinUI.Capabilities
{
    public sealed partial class InputInjectionCapability
    {
        ...
        private async Task<string> InjectKeyPressAsync(string argsJson)
        {
            ...
            await _runOnUiThread(() => { ActivateWindow(); return Task.CompletedTask; }).ConfigureAwait(false);
        ...
        private async Task<string> InjectTextAsync(string argsJson)
        {
            ...
            // Bring window to foreground before injecting keyboard input, so keys
            // reach the correct focused control in the WinUI app rather than VS Code.
            await _runOnUiThread(() => { ActivateWindow(); return Task.CompletedTask; }).ConfigureAwait(false);
            await Task.Delay(60).ConfigureAwait(false);
