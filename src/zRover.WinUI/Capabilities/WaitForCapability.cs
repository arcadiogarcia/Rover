...
namespace zRover.WinUI.Capabilities
{
    ...
    internal sealed class WaitForCapability : IDebugCapability
    {
        ...
        {
            _context = context;
            // The root element is stored by ScreenshotCapability; we get it at capture time via context
    ...
    ""pattern"": { ""type"": ""string"", ""description"": ""[log_match] Substring or regex matched (case-insensitive) against log message and category."" }
        ...
        private async Task<ulong> CaptureFrameHashAsync()
        {
            ...
            if (_context!.RunOnUiThread != null)
            {
                ...
                {
                    var root = DebugHost.CurrentWindow?.Content as Microsoft.UI.Xaml.UIElement;
                    if (root != null)
                        bitmap = await ScreenshotAnnotator.CaptureUiAsBitmapAsync(root).ConfigureAwait(false);
